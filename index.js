...
async function starts() {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			const ownerNumber = ["5511946462131@s.whatsapp.net"] // replace this with your number
			...
			switch(command) {
				...
				case 'gtts':
					...
					if (args.length < 2) return client.sendMessage(from, 'Cadê o texto porra', text, {quoted: mek})
					dtt = body.slice(9)
					ranm = getRandom('.mp3')
					rano = getRandom('.ogg')
					dtt.length > 600
					? reply('A maior parte do texto é gay irmão')
				...
				case 'canal':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '️*canal:* sem vídeos no pornhub no momento \n\n'})
					break
				case 'mia1':
					memein = await kagApi.memeindo()
